...
export default function HotelRoomCard({ room }) {
  ...
  return (
    <Link
      to={`/room/${room._id}`}
      className="flex flex-col md:flex-row my-4 py-4 gap-5 w-full"
    >
      ...
      <div className="flex flex-col gap-3">
        ...
        <p>
          <span className="font-playfair text-2xl">{name}</span>
        </p>
        ...
        <div className=" flex flex-wrap gap-3">
          {amenities.map((amenity, index) => {
            ...
            return (
              <div
                key={index}
                className=" flex flex-row items-center gap-1.5 bg-[#F0F0F7] p-2 rounded-xl text-xs"
              >
